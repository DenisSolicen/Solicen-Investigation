...
Однако сначала о самом видео: то, что Куплинов опубликовал в 15:00, может быть выложено в качестве извинения перед пользователями, которым пришлось терпеть выходки и работу ВК на его последних видео, в течение многих последних дней.
...
Если всё так, то это не решает проблему в целом, а лишь скрывает её для конкретной части пользователей, если в баг попадет видео пользователя у которого нет особых связей с поддержкой ВК, то он может и не увидеть своё видео в рабочем состоянии на платформе.
...
9. **Авторам приходится решать проблемы самостоятельно.** Если в процессе обработки, что-то пойдет не так, ВК не начнет автоматическую перекодировку, нет, автору придется делать перезалив вручную, когда и если он заметит аномалию.
...
На момент обновления (31 марта, 20:00):
